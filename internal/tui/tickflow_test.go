...
func TestDispatch_skipReturnsSkip(t *testing.T) {
	got, err := dispatchAction(t.Context(), ActionSkip, &fakeReader{}, "ABC", nil, io.Discard)
...
func TestDispatch_cancelledReturnsCancelled(t *testing.T) {
	got, err := dispatchAction(t.Context(), ActionCancelled, &fakeReader{}, "ABC", nil, io.Discard)
	require.NoError(t, err)
	assert.Equal(t, ActionCancelled, got.Action)
}

func TestDispatch_createReturnsHint(t *testing.T) {
	got, err := dispatchAction(t.Context(), ActionCreate, &fakeReader{}, "ABC", nil, io.Discard)
	require.NoError(t, err)
	assert.Equal(t, ActionCreate, got.Action)
	assert.Contains(t, got.HintText, "arat ticket create")
}
...
func TestDispatch_pickFetchesAndPicks(t *testing.T) {
	...
	got, err := dispatchAction(t.Context(), ActionPick, lr, "ABC", picker, io.Discard)
	require.NoError(t, err)
...
func TestDispatch_pickEmptyListGivesHint(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, ActionCreate, got.Action)
	assert.Contains(t, got.HintText, "no open issues")
}

func TestDispatch_pickListError(t *testing.T) {
	lr := &fakeReader{err: errors.New("boom")}
	_, err := dispatchAction(t.Context(), ActionPick, lr, "ABC", nil, io.Discard)
	require.Error(t, err)
}
...
func TestDispatch_pickerError(t *testing.T) {
	...
	_, err := dispatchAction(t.Context(), ActionPick, lr, "ABC", picker, io.Discard)
	require.Error(t, err)
}
