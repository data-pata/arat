...
| `arat new [name] [--project] [--in REF] [--ticket TKT \| --no-ticket] [--repos a,b] [--from-current] [--carry-context] [--carry-session ID] [--code-workspace]` | Create workspace + worktrees + CLAUDE.md. Without `--ticket`/`--no-ticket` and on a tty: opens an interactive ticket flow. The name is optional when a ticket supplies one: it is derived from the issue title (slugified, ticket id prefixed as usual), shown in a pre-filled prompt on a tty (Enter accepts, Esc cancels; after skipping the ticket you type a name or the command cancels), and derived silently for `--ticket`/`--new-ticket`, so `arat new --ticket abc-12` alone works. `--project` creates a top-level container instead of a leaf; `--in <ref>` places the new workspace inside another one (`--in .` for the workspace at cwd), and `--from-parent` branches off that parent's branches. `--carry-session` moves a Claude Code session jsonl into the new workspace's project dir so `/resume` finds it after `cd`. |
